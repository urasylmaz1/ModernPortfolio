...
public class TestimonialService : ITestimonialService
{
    ...
    private void ValidateTestimonial(Testimonial testimonial)
    {
        ...
        {
            throw new ArgumentException("Testimonial client name cannot be empty or whitespace!", nameof(testimonial));
        }
        if (testimonial.ClientName.Length > 100)
        {
            throw new ArgumentException("Testimonial client name cannot exceed 100 characters", nameof(testimonial));            
        }

        //ClientPosition
        if (string.IsNullOrWhiteSpace(testimonial.ClientPosition) && testimonial.ClientPosition.Length > 100)
        {
            throw new ArgumentException("Testimonial client position cannot exceed 100 characters", nameof(testimonial));            
        }
        //Comment
        if (string.IsNullOrWhiteSpace(testimonial.Comment))
        {
            throw new ArgumentException("Testimonial comment cannot be empty or whitespace!", nameof(testimonial));
        }

        //ClientImageURL
        if(!string.IsNullOrWhiteSpace(testimonial.ClientImageUrl) && testimonial.ClientImageUrl.Length > 500)
        {
            throw new ArgumentException("Testimonial image url cannot exceed 500 characters", nameof(testimonial));
        }
